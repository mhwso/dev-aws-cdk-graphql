...
export class DevAwsCdkGraphqlStack extends cdk.Stack {
	constructor(scope: Construct, id: string, props: ExtendedProperties) {
		...
		// jwt to api when call from app to backend

		const api = new GraphqlApi(this, 'cdk-app', {
			name: `${prefix}-cdk-api`,
			logConfig: {
		...
		const usEast1Cert = Certificate.fromCertificateArn(this, 'CertificateImportedFromUsEast1', props.certArnUsEast1);
